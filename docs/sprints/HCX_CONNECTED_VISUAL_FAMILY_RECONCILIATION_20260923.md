...
Live preview review also exposed the account, invitation, pricing, legal and contact return links pointing to the production origins. The first temporary preview surfaced public navigation pointing to the two production origins. `siteOrigins.ts` now resolves both public and app links to the same pages.dev origin only on the existing Cloudflare Pages preview domain. The account, invitation, pricing and information-page destinations now use the same origin resolver, keeping visual review and ordinary preview navigation inside the isolated reconciliation environment. The production domains retain their canonical split, and the preview backend is still pinned to E3 in `src/lib/supabase.ts`; no DNS or provider setting changed. A direct origin mapping regression test covers this boundary. The first main-targeted Front Door Visual Review gate caught a 166px mobile footer against its 120px limit. The shared mobile logo/type/legal stack was compacted and must be rerendered and recertified; the failing candidate cannot be treated as certified.
