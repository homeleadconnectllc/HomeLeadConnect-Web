...
const { data: submitted, error: submitError } = await client.rpc("submit_professional_application", {
  p_form_slug: "professional-application", p_request_id: requestId,
  p_organization_name: marker, p_contact_name: "HomeLead Connect E2E", p_email: email,
  p_phone: "7175519897", p_trade_categories: "E2E Test Painting", p_service_territory: "Harrisburg, PA",
...
const { error: serviceError } = await client.rpc("add_linked_provider_service", { p_contractor_id: contractorId, p_service_name: "E2E Test Painting 20260909" });
if (serviceError) throw serviceError;
const { error: availabilityError } = await client.rpc("set_linked_provider_availability", {
  p_contractor_id: contractorId, p_available: true, p_note: "E2E production verification only",
...
const { data: decision, error: decisionError } = await client.rpc("contractor_decide_assignment", { p_assignment_id: assignment.id, p_decision: "accepted" });
...
const { data: progressId, error: progressError } = await client.rpc("contractor_record_job_progress", {
  p_assignment_id: assignment.id, p_status: "completed", p_note: "E2E production lifecycle verification completed.",
});
