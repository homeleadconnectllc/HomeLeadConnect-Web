  ...
  for (const width of widths) {
    ...
    for (const path of routes) {
      ...
      try {
        ...
        check(() => assert.ok(measured.visibleBrandImage, 'Approved master logo is not physically visible'));
        check(() => assert.equal(measured.visibleBrandImage?.src, expectedLogoPath, 'Visible navbar artwork is not the approved master logo'));
        check(() => assert.ok(measured.visibleBrandImage?.width >= 40 && measured.visibleBrandImage?.height >= 40, 'Approved master logo is rendered too small to be visibly present'));
