...
async function metrics(page) {
  return page.evaluate(() => {
    const header = document.querySelector('[data-hlc-public-navigation], .hlc-public-shared-nav, .hlc-public-site-nav');
...
try {
  await Promise.all(widths.map(async (width) => {
    ...
    await authorityPage.locator('[data-hlc-public-navigation], .hlc-public-shared-nav, .hlc-public-site-nav').waitFor();
    ...
    for (const path of routes) {
      ...
      try {
        ...
        await page.locator('[data-hlc-public-navigation], .hlc-public-shared-nav, .hlc-public-site-nav').waitFor({ timeout: 10000 });
        ...
        const header = page.locator('[data-hlc-public-navigation], .hlc-public-shared-nav, .hlc-public-site-nav');
