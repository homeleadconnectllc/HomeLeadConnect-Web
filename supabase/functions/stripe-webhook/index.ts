...
Deno.serve(async(request)=>{
  ...
  try{
    if(event.type==="checkout.session.completed"){
      ...
      const session=event.data.object as Stripe.Checkout.Session;if(session.metadata?.hlc_payment_kind==="resident_job"){const paymentIntentId=typeof session.payment_intent==="string"?session.payment_intent:session.payment_intent?.id||"";const{error}=await admin.rpc("set_resident_job_payment_provider_state",{p_checkout_session_id:session.id,p_payment_intent_id:paymentIntentId,p_status:"cancelled",p_receipt_url:null,p_failure_code:"stripe_checkout_expired",p_failure_message:"The secure checkout expired before payment completed. Start a fresh checkout to continue."});if(error)throw error;}
      ...
      if(paymentIntentId&&charge.amount_refunded>=charge.amount){const{data:payment,error:paymentError}=await admin.from("resident_job_payments").select("external_checkout_session_id").eq("external_payment_intent_id",paymentIntentId).maybeSingle();if(paymentError)throw paymentError;if(payment?.external_checkout_session_id){const{error}=await admin.rpc("set_resident_job_payment_provider_state",{p_checkout_session_id:payment.external_checkout_session_id,p_payment_intent_id:paymentIntentId,p_status:"refunded",p_receipt_url:charge.receipt_url||null,p_failure_code:null,p_failure_message:null});if(error)throw error;}}
