-- Provider-neutral communication routing for #153.
-- Preserve the existing RPC signature while resolving transport from workspace
-- configuration instead of hard-coding a vendor by channel.
...
begin
  ...
  select pc.provider_name, pc.status
    into v_provider_name, v_provider_status
  from public.communication_provider_connections pc
  where pc.workspace_id=v_workspace_id
    and pc.channel=lower(p_channel)
    and pc.status in ('connected','manual_available')
  order by
    case pc.status when 'connected' then 0 else 1 end,
    pc.verified_at desc nulls last,
  ...
  v_status := case
    when v_decision='ALLOW' and v_provider_status='manual_available' then 'review'
    when v_decision='ALLOW' then 'queued'
    when v_decision='REVIEW' then 'review'
    else 'blocked'
  end;
  ...
  return jsonb_build_object(
    ...
    'provider_status',coalesce(v_provider_status,'unconfigured'),
