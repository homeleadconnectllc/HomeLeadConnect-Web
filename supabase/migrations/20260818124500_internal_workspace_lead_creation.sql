-- Add a controlled authenticated RPC for staff-created CRM leads while preserving
-- public.leads as a server-only INSERT surface.
--
-- The wrapper derives the active workspace from the authenticated profile, verifies
-- canonical workspace membership and role, enforces the plan lead limit for new
-- phone identities, validates contact fields, and delegates the write to the
-- canonical causal single-writer function.
...
begin
  ...
  v_lead_id := causal.ingest_lead(
    ...
    p_source => 'manual_crm',
    p_priority => case when v_existing_lead_id is null then 'medium' else null end,
    p_archived => case when v_existing_lead_id is null then false else null end,
