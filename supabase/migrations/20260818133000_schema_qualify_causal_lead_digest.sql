-- Repair the canonical causal lead writer discovered by rollback-only production
-- certification. pgcrypto is installed in the extensions schema while the
-- SECURITY DEFINER function intentionally pins search_path to public.
--
-- Keep the locked search path and schema-qualify only the cryptographic primitive
-- instead of broadening function name resolution.
