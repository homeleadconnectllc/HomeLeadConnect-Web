-- Post-launch foundation for document OCR/extraction review.
-- This migration defines storage for proposed processing output only.
-- It does not install an OCR provider, auto-post financial data, or mutate source documents.
...
begin
  ...
  insert into public.document_processing_jobs(
    workspace_id,document_id,processing_kind,status,requested_by
  ) values (
    v_document.workspace_id,v_document.id,v_kind,'queued',auth.uid()
...
begin
  ...
  if not found then
    raise exception 'Extraction field not found.' using errcode='P0002';
  end if;
...
-- Extend the document audit vocabulary for processing/review evidence.
...
alter table public.document_events
  ...
  check (action in ('uploaded','shared','viewed','deleted','processing_requested','processing_completed','processing_failed','extraction_reviewed'));
