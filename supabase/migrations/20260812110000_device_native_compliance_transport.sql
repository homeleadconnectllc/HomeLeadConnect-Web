-- Allow phone-native call/SMS handoff to use the existing HLC compliance engine
-- without pretending the user's device is a configured communications provider.
...
begin
  ...
  if v_provider_name <> 'device_native' and not exists(select 1 from public.communication_provider_connections pc where pc.workspace_id=v_workspace_id
  ...
  return jsonb_build_object('id',v_check_id,'decision',v_decision,'reasons',v_reasons,'provider_ready',v_provider_name='device_native' or not(v_reasons?'provider_not_connected'));
end; $$;
