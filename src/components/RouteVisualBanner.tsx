...
const publicAppRoutes = new Set([
  "/app", "/portal", "/contact", "/request-service", "/about", "/homeowners", "/contractors", "/how-it-works",
  "/leadscope", "/community", "/services", "/pricing", "/trust", "/professionals", "/partners", "/demo",
  "/professional-application", "/accessibility", "/privacy", "/terms", "/platform-disclosure", "/login", "/register",
  "/forgot-password", "/reset-password", "/portal/accept", "/team/accept",
]);

const routeVisuals: Array<{ match: (pathname: string) => boolean; visual: RouteVisual }> = [
  { match: (p) => p === "/dashboard" || p === "/start-here" || p === "/ecosystem", visual: { eyebrow: "LIVE COMMAND", title: "See the business before you work the business.", detail: "Live priorities, product areas and HLC intelligence organized into one visual command layer.", action: { label: "Open workflow", to: "/workflow" }, tone: "command" } },
  { match: (p) => p === "/work" || p === "/workflow" || p === "/automations" || p === "/activity" || p === "/partners/manage" || p.startsWith("/leads") || p.startsWith("/jobs") || p === "/estimator" || p.startsWith("/follow-ups") || p === "/calendar" || p === "/call-center" || p === "/manual-communications", visual: { eyebrow: "OPERATIONS", title: "Move every request toward completion.", detail: "See where the work sits, what is blocking it and what action moves it forward.", action: { label: "Work home", to: "/work" }, tone: "work" } },
  ...
  { match: (p) => p.startsWith("/settings") || p === "/profile" || p === "/team" || p.startsWith("/homeowner-portal") || p.startsWith("/contractor-portal") || p.startsWith("/partner-portal") || p === "/notifications" || p === "/messages", visual: { eyebrow: "ACCOUNT & ACCESS", title: "Keep identity, access and communication understandable.", detail: "Account health, people, messages, billing and portal activity with clear visual status and next actions.", action: { label: "Open settings", to: "/settings" }, tone: "account" } },
  { match: (p) => publicAppRoutes.has(p), visual: { eyebrow: "HOMELEAD CONNECT", title: "One front door into the HLC ecosystem.", detail: "A clear, visual path into home help, professional access, trust information and the connected HLC workspace.", action: { label: "Open HLC", to: "/app" }, tone: "public" } },
];
