...
export async function uploadDocument(input:{entityType:string;entityId:string;sharingScope:string;file:File}){
 ...
 const requestedEntityId=input.entityId.trim();
 const relatedEntityType=requestedEntityId?input.entityType:"workspace";
 const relatedEntityId=requestedEntityId||workspace;
 const name=input.file.name.replace(/[^a-zA-Z0-9._-]/g,"_");const path=`${workspace}/${crypto.randomUUID()}-${name}`;
 ...
 const {data,error}=await supabase.rpc("register_document",{p_entity_type:relatedEntityType,p_entity_id:relatedEntityId,p_filename:input.file.name,p_storage_path:path,p_mime_type:input.file.type,p_byte_size:input.file.size,p_sharing_scope:input.sharingScope});
