...
export async function resolveReport(id:string,status:"resolved"|"dismissed"){const{data:{user},error:userError}=await supabase.auth.getUser();if(userError)throw userError;if(!user)throw new Error("Authentication is required.");const{data,error}=await supabase.from("community_reports").update({status,resolved_at:new Date().toISOString(),resolved_by:user.id}).eq("id",id).select("id,post_id,review_id,reason,status,created_at").single();if(error)throw error;return data as CommunityReport}
