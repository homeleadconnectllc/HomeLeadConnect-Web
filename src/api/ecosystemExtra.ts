...
export async function getWorkspaceAnalytics(){const{workspaceId}=await context();const [leads,jobs,appointments,contractors,followups]=await Promise.all([
  ...
  for(const r of [leads,jobs,appointments,contractors,followups]) if(r.error) throw r.error;
  return{leads:leads.count??0,jobs:jobs.count??0,appointments:appointments.count??0,contractors:contractors.count??0,followUps:followups.count??0};
}
