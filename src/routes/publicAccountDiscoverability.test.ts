...
test("parser-seeded public shell preserves the Connected Experience first-paint contract", () => {
  ...
  assert.match(index, /aria-label="HomeLead Connect home"[^>]*>[\s\S]*data-hlc-master-logo="true"[^>]+src="\/brand\/homelead-connect-master-transparent\.png"[\s\S]*HomeLead Connect[\s\S]*<\/a>/);
