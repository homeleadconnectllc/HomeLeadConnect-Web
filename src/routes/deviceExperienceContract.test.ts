...
const desktopShell = readFileSync("src/styles/desktop-workspace-shell.css", "utf8");
...
test("normal laptop widths keep desktop workspace navigation available with an explicit collapse control", () => {
  ...
  assert.match(authenticatedStyles, /desktop-workspace-shell\.css/);
  assert.doesNotMatch(authenticatedStyles, /desktop-shell-contract-v2\.css/);
  assert.doesNotMatch(authenticatedStyles, /global-readability-certification\.css/);
  assert.match(desktopShell, /@media \(min-width:1025px\)/);
  assert.match(desktopShell, /grid-template-columns:336px minmax\(0,1fr\)/);
  assert.match(desktopShell, /\.hlc-signed-in-shell\.hlc-sidebar-collapsed/);
  assert.match(desktopShell, /\.hlc-desktop-sidebar-toggle/);
});
