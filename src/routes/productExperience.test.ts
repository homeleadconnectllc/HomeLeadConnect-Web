...
test("routed HLC content keeps responsive containment without forced global centering", () => {
  ...
  assert.doesNotMatch(responsiveContract, /text-align:\s*center\s*!important/);
});
...
test("desktop workspace navigation uses the current structural shell without a permanent AI rail", () => {
  assert.match(authenticatedEntry, /desktop-workspace-shell\.css/);
  assert.match(desktopWorkspaceShell, /@media \(min-width:1025px\)/);
  assert.match(desktopWorkspaceShell, /grid-template-columns:336px minmax\(0,1fr\)/);
  assert.match(desktopWorkspaceShell, /\.hlc-navbar-links[\s\S]*overflow-y:auto/);
  assert.doesNotMatch(authenticatedEntry, /desktop-agent-team-rail\.css/);
});

test("the proactive briefing remains component-owned instead of depending on retired release paint", () => {
  ...
  assert.doesNotMatch(authenticatedEntry, /mobile-release-fix\.css/);
});

test("open agent panel remains explicit and viewport-owned", () => {
...
test("signed-in mobile navigation behaves like an adaptive field app", () => {
  ...
  assert.match(mobileShellClosure, /\.hlc-mobile-tabbar[\s\S]*position:fixed!important/);
  assert.match(mobileShellClosure, /safe-area-inset-bottom/);
...
test("Kendrell HQ separates the family memorial from the operational AI workspace", () => {
  ...
  assert.match(agentTeamCss, /@media \(max-width: 820px\)/);
...
test("retired premium presentation layers stay disconnected from current runtime", () => {
  for (const retired of [
    "premium-theme.css",
    "premium-effects.css",
    "global-premium-system.css",
    "global-visual-pizzazz.css",
    "contrast-contract.css",
    "workspace-premium-v3.css",
  ]) {
    assert.ok(!mainEntry.includes(retired));
    assert.ok(!authenticatedEntry.includes(retired));
  }
});

test("current interaction behavior does not depend on the retired premium-effects layer", () => {
  assert.doesNotMatch(mainEntry, /premium-effects\.css/);
  assert.doesNotMatch(authenticatedEntry, /premium-effects\.css/);
  assert.match(contextualDock, /aria-label/);
});

test("readability no longer depends on a retired global contrast repaint", () => {
  assert.doesNotMatch(mainEntry, /contrast-contract\.css/);
  assert.doesNotMatch(authenticatedEntry, /contrast-contract\.css/);
  assert.match(responsiveContract, /--hlc-readable-max:860px/);
});
...
test("mobile agent guidance remains a complete portaled workflow without retired release-theme dependency", () => {
