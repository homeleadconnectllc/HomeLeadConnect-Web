...
export default function FollowUps() {
  ...
  async function load() {
    ...
    setItems(followUps); setLeads(leadRows);
  }

  useEffect(() => {
    Promise.all([listFollowUps(), listLeads()])
      .then(([followUps, leadRows]) => { setItems(followUps); setLeads(leadRows); })
  ...
  async function create(event: FormEvent) {
    ...
    await run(async () => {
      ...
      setScheduledFor(""); setNotes(""); await load();
    }, "Follow-up created.");
  }

  async function complete(item: FollowUp) {
    await run(async () => { await completeFollowUp(item.id); await load(); }, "Follow-up completed.");
  }

  async function run(action: () => Promise<void>, success: string) {
    setBusy(true); setError(""); setMessage("");
    try { await action(); setMessage(success); }
    catch (reason) { setError(errorMessage(reason, "Unable to update the follow-up.")); }
  ...
  return <main style={pageStyle}>
    ...
    <form onSubmit={create} style={cardStyle}>
      <label>Lead<select required value={leadId} onChange={(event) => setLeadId(event.target.value)}>
        <option value="">Select a lead</option>
        {leads.map((lead) => <option key={lead.id} value={lead.id_uuid}>{lead.full_name || lead.phone}</option>)}
      </select></label>
      <label>Due date and time<input required type="datetime-local" value={scheduledFor} onChange={(event) => setScheduledFor(event.target.value)} /></label>
