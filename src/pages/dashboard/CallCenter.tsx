...
export default function CallCenter() {
  ...
  async function saveDisposition() {
    ...
    try {
      ...
      const evidence = [
        notes.trim(),
        `HLC disposition: ${selectedDisposition.label}`,
    ...
    <main className="hlc-call-center-workspace">
      <header className="hlc-call-center-header">
        <div>
          ...
          <h1>Call Center</h1>
          <p>Run carrier handoffs, connected company lines, persisted call history, intelligent outcomes, scripts, rebuttals and follow-up evidence from one HLC communications workspace.</p>
        </div>
      ...
      <section className="hlc-call-carrier-lane" aria-labelledby="call-intelligence-heading">
        <div className="hlc-call-section-heading"><div><span>OPERATOR INTELLIGENCE</span><h2 id="call-intelligence-heading">Scripts, rebuttals & dispositions</h2></div><strong>HLC guided</strong></div>
        <p className="hlc-call-carrier-explainer">Use the approved HLC language during the conversation, then choose the outcome that best matches what actually happened. The disposition carries recommended next actions, workflow context and automation keys into the persisted call evidence.</p>
        ...
        <div className="hlc-call-action-rail">
          <Link to="/help">Open Resources & manuals</Link>
          <Link to="/follow-ups">Open follow-ups</Link>
          <Link to="/workflow">Open Golden Workflow</Link>
        </div>
      </section>
      ...
      {!loading && googleVoicePhone && (
        <section className="hlc-call-carrier-lane" aria-labelledby="google-voice-companion-heading">
          <div className="hlc-call-section-heading">
            <div><span>ACTIVE CARRIER LANE</span><h2 id="google-voice-companion-heading">Google Voice</h2></div>
            <div className="hlc-call-carrier-number"><strong>{formatPhoneNumber(googleVoicePhone.phone_number)}</strong><small>{googleVoicePhone.is_primary ? "Primary HLC company line" : "HLC company line"} · {googleVoicePhone.verification_state}</small></div>
          </div>
          <p className="hlc-call-carrier-explainer">Keep HLC open for customer context, compliance checks, notes, outcomes and follow-up while Google Voice handles the live carrier session. HLC never marks a call answered, completed or transferred unless operator or provider evidence records that outcome.</p>
          <div className="hlc-call-carrier-grid">
            <div className="hlc-call-device-readiness" aria-label="Google Voice device readiness">
              ...
              {deviceMode === "ios" ? <><span>1. Keep the Google Voice app signed in and allow Voice notifications.</span><span>2. For calls that must originate from your Google Voice number, start the call in Google Voice rather than the iPhone Phone app.</span><span>3. Return to HLC after the call to record the outcome and create follow-up work.</span></> : <><span>1. Keep voice.google.com signed in and open in a supported browser.</span><span>2. Allow browser/system notifications so incoming Voice activity can alert you.</span><span>3. Keep HLC alongside Google Voice for lead context, history and follow-up.</span></>}
            </div>
            <div className="hlc-call-integration-ledger" aria-label="Google Voice integration status">
              <span><strong>Carrier</strong><small>Google Voice</small></span><span><strong>Live ringing</strong><small>Google Voice app/web</small></span><span><strong>Lead context</strong><small>HLC</small></span><span><strong>Compliance + history</strong><small>HLC</small></span><span><strong>Outcome + follow-up</strong><small>HLC</small></span><span><strong>Direct Voice call control API</strong><small>Not connected</small></span>
            </div>
          </div>
          ...
          <p className="hlc-call-boundary-note">Google Voice remains the carrier surface. Features available inside Google Voice depend on the Voice account and plan. HLC does not provide embedded Answer, Hold, Transfer, Hang Up controls and does not claim direct recording, delivery or inbound synchronization unless a supported provider integration supplies that evidence.</p>
        </section>
      )}
      ...
      <section className="hlc-call-lines" aria-labelledby="business-numbers-heading">
        ...
        <div className="hlc-call-line-list">
          {!loading && phones.length === 0 ? <p className="hlc-call-empty">No connected phone line yet. HLC can use a supported provider or device-native/manual communication path once configured.</p> : phones.map((phone) => (
            <article className="hlc-call-line-row" key={phone.id}>
              <div className="hlc-call-line-identity"><strong>{phone.display_name}: {formatPhoneNumber(phone.phone_number)}</strong><small>{providerLabel(phone.provider_type)} · {phone.readiness_state.replaceAll("_", " ")}{phone.is_primary ? " · Primary HLC number" : ""}</small></div>
              ...
              {phone.provider_type === "google_voice" && !phone.browser_calling_enabled && <p className="hlc-call-line-note">Google Voice is the active HLC carrier for this line. HLC prepares contact and compliance context, opens the carrier surface, and preserves operator-reported history. Live Google Voice call controls stay inside Google Voice unless a supported API is connected later.</p>}
