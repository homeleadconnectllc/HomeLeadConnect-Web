...
export default function ResourcesWorkspace() {
  ...
  return <main className="hlc-sourcing-workspace" data-live-price={String(RESOURCE_TRUTH_BOUNDARY.livePriceData)}>
    <header className="hlc-sourcing-hero"><div><p>E4 · DION SOURCING DESK</p><h1>{mode === "materials" ? "Material planning" : mode === "map" ? "Supplier map & routing" : mode === "suppliers" ? "Supplier discovery" : "What are you working on?"}</h1><span>Move from visual category discovery to supplier research, material planning, routing, and saved resources without confusing external merchant information with verified HLC data.</span></div><aside><strong>CHECK CURRENT DETAILS</strong><small>HLC does not receive live merchant price or inventory data. Confirm specifications, price, stock, delivery, returns, and route details with the supplier.</small></aside></header>
    <nav className="hlc-sourcing-nav" aria-label="Resources navigation"><Link className={mode==="home"?"is-active":""} to="/resources">Resources</Link><Link className={mode==="suppliers"?"is-active":""} to="/resources/suppliers">Suppliers</Link><Link className={mode==="map"?"is-active":""} to="/resources/suppliers/map">Map & routing</Link><Link className={mode==="materials"?"is-active":""} to="/resources/materials">Materials</Link><Link to="/resources/forms">Forms</Link><Link to="/documents">Documents</Link></nav>
