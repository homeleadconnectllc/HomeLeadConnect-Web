...
function Moderation(){
  ...
  return <><form onSubmit={submit} style={cardStyle}><h2>Report Community content</h2><p>Select the actual Community item so the report is linked to the correct record. Internal post IDs are never required from the operator.</p><label>Community item<select required value={postId} onChange={e=>setPostId(e.target.value)}><option value="">Choose content</option>{posts.map(post=><option key={post.id} value={post.id}>{post.kind} · {post.title}</option>)}</select></label>{posts.length===0&&<p>No active Community content is available to report.</p>}<label>Reason<textarea required value={reason} onChange={e=>setReason(e.target.value)}/></label><button disabled={busy||posts.length===0}>{busy?"Saving…":"Submit report"}</button>{error&&<p role="alert">{error}</p>}</form><section style={gridStyle}>{items.map(x=><article key={x.id} style={cardStyle}><strong>{x.status}</strong><p>{x.reason}</p>{x.status==="open"&&<div style={rowStyle}><button type="button" disabled={busy} onClick={()=>void close(x.id,"resolved")}>Resolve</button><button type="button" disabled={busy} onClick={()=>void close(x.id,"dismissed")}>Dismiss</button></div>}</article>)}{items.length===0&&!error&&<p>No reports visible.</p>}</section></>;
}
