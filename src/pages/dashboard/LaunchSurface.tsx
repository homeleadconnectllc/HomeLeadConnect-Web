...
export default function LaunchSurface({ page }: { page: LaunchSurfaceKey }) {
  return <main className="hlc-launch-surface">
    <header className="hlc-launch-hero">
      <h1 className="hlc-launch-heading-reset">{titles[page]}</h1>
    </header>
    <Surface page={page} />
  </main>;
}
...
function Posts({ kind }: { kind?: CommunityPost["kind"] }) {
  ...
  return <>
    <form onSubmit={submit} className="hlc-launch-card"><h2>Publish {chosen}</h2>
    ...
    <section className="hlc-launch-grid">{items.map(item=><article key={item.id} className="hlc-launch-card"><small>{item.kind.toUpperCase()}</small><h2>{item.title}</h2><p>{item.body}</p>{item.event_at&&<p>{new Date(item.event_at).toLocaleString()}</p>}</article>)}{items.length===0&&<p>No Community items yet.</p>}</section>
  </>;
}
...
function Reviews(){
  ...
  return <><form onSubmit={submit} className="hlc-launch-card"><h2>Write a verified review</h2><label>Completed job<select required value={jobId} onChange={e=>setJobId(e.target.value)}><option value="">Choose job</option>{jobs.map(j=><option key={j.id} value={j.id}>{j.name}</option>)}</select></label><label>Rating<select value={rating} onChange={e=>setRating(e.target.value)}>{[5,4,3,2,1].map(n=><option key={n}>{n}</option>)}</select></label><label>Review<textarea required rows={4} value={body} onChange={e=>setBody(e.target.value)}/></label><button>Publish review</button>{error&&<p role="alert">{error}</p>}</form><section className="hlc-launch-grid">{reviews.map(r=><article key={r.id} className="hlc-launch-card"><strong>{"★".repeat(r.rating)}</strong><p>{r.body}</p><small>Job {r.job_id}</small></article>)}{reviews.length===0&&<p>No eligible reviews yet.</p>}</section></>;
}

function Referrals(){const[items,setItems]=useState<any[]>([]);const[email,setEmail]=useState("");const[note,setNote]=useState("");const[error,setError]=useState("");async function load(){try{setItems(await listReferrals())}catch(e){setError(errorMessage(e,"Unable to load referrals."))}}useEffect(()=>{void load()},[]);async function submit(e:FormEvent){e.preventDefault();try{await createReferral(email,note);setEmail("");setNote("");await load()}catch(r){setError(errorMessage(r,"Unable to record referral."))}}return <><form onSubmit={submit} className="hlc-launch-card"><h2>Record referral intent</h2><p>This records attribution only and does not automatically message or enroll another person.</p><label>Email<input type="email" required value={email} onChange={e=>setEmail(e.target.value)}/></label><label>Note<textarea value={note} onChange={e=>setNote(e.target.value)}/></label><button>Record referral</button>{error&&<p role="alert">{error}</p>}</form><section className="hlc-launch-card">{items.map(x=><p key={x.id}><strong>{x.referred_email}</strong> · {x.status}</p>)}{items.length===0&&<p>No referrals recorded.</p>}</section></>}

function Moderation(){
  const[items,setItems]=useState<any[]>([]); const[posts,setPosts]=useState<CommunityPost[]>([]); const[postId,setPostId]=useState(""); const[reason,setReason]=useState(""); const[error,setError]=useState(""); const[busy,setBusy]=useState(false);
  async function load(){try{const[reports,communityPosts]=await Promise.all([listReports(),listCommunityPosts()]);setItems(reports);setPosts(communityPosts)}catch(e){setError(errorMessage(e,"Moderation queue is restricted."))}}
  useEffect(()=>{void load()},[]);
  async function submit(e:FormEvent){e.preventDefault();if(!postId)return;setBusy(true);setError("");try{await createReport({postId,reason});setPostId("");setReason("");await load()}catch(r){setError(errorMessage(r,"Unable to submit report."))}finally{setBusy(false)}}
  async function close(id:string,status:"resolved"|"dismissed"){setBusy(true);setError("");try{await resolveReport(id,status);await load()}catch(r){setError(errorMessage(r,"Only an authorized workspace owner may resolve reports."))}finally{setBusy(false)}}
  return <><form onSubmit={submit} className="hlc-launch-card"><h2>Report Community content</h2><p>Select the actual Community item so the report is linked to the correct record. Internal post IDs are never required from the operator.</p><label>Community item<select required value={postId} onChange={e=>setPostId(e.target.value)}><option value="">Choose content</option>{posts.map(post=><option key={post.id} value={post.id}>{post.kind} · {post.title}</option>)}</select></label>{posts.length===0&&<p>No active Community content is available to report.</p>}<label>Reason<textarea required value={reason} onChange={e=>setReason(e.target.value)}/></label><button disabled={busy||posts.length===0}>{busy?"Saving…":"Submit report"}</button>{error&&<p role="alert">{error}</p>}</form><section className="hlc-launch-grid">{items.map(x=><article key={x.id} className="hlc-launch-card"><strong>{x.status}</strong><p>{x.reason}</p>{x.status==="open"&&<div className="hlc-launch-row"><button type="button" disabled={busy} onClick={()=>void close(x.id,"resolved")}>Resolve</button><button type="button" disabled={busy} onClick={()=>void close(x.id,"dismissed")}>Dismiss</button></div>}</article>)}{items.length===0&&!error&&<p>No reports visible.</p>}</section></>;
}

function Groups(){const[items,setItems]=useState<any[]>([]);const[name,setName]=useState("");const[description,setDescription]=useState("");const[error,setError]=useState("");async function load(){try{setItems(await listCommunityGroups())}catch(e){setError(errorMessage(e,"Unable to load groups."))}}useEffect(()=>{void load()},[]);async function submit(e:FormEvent){e.preventDefault();try{await createCommunityGroup(name,description);setName("");setDescription("");await load()}catch(r){setError(errorMessage(r,"Unable to create group."))}}return <><form onSubmit={submit} className="hlc-launch-card"><h2>Create group</h2><label>Name<input required value={name} onChange={e=>setName(e.target.value)}/></label><label>Description<textarea value={description} onChange={e=>setDescription(e.target.value)}/></label><button>Create</button>{error&&<p role="alert">{error}</p>}</form><section className="hlc-launch-grid">{items.map(g=><article key={g.id} className="hlc-launch-card"><h2>{g.name}</h2><p>{g.description||"No description."}</p></article>)}{items.length===0&&<p>No groups yet.</p>}</section></>}

function Properties(){const[items,setItems]=useState<any[]>([]);const[form,setForm]=useState({label:"",address:"",city:"",state:"PA",zip:""});const[error,setError]=useState("");async function load(){try{setItems(await listResidentProperties())}catch(e){setError(errorMessage(e,"Unable to load properties."))}}useEffect(()=>{void load()},[]);async function submit(e:FormEvent){e.preventDefault();try{await createResidentProperty(form);setForm({label:"",address:"",city:"",state:"PA",zip:""});await load()}catch(r){setError(errorMessage(r,"Unable to save property."))}}return <><form onSubmit={submit} className="hlc-launch-card"><h2>Add property/service location</h2><label>Label<input required value={form.label} onChange={e=>setForm({...form,label:e.target.value})}/></label><label>Address<input value={form.address} onChange={e=>setForm({...form,address:e.target.value})}/></label><label>City<input value={form.city} onChange={e=>setForm({...form,city:e.target.value})}/></label><label>State<input value={form.state} onChange={e=>setForm({...form,state:e.target.value})}/></label><label>ZIP<input value={form.zip} onChange={e=>setForm({...form,zip:e.target.value})}/></label><button>Save property</button>{error&&<p role="alert">{error}</p>}</form><section className="hlc-launch-grid">{items.map(p=><article key={p.id} className="hlc-launch-card"><h2>{p.label}</h2><p>{[p.address,p.city,p.state,p.zip].filter(Boolean).join(", ")||"Address not provided"}</p></article>)}{items.length===0&&<p>No properties saved.</p>}</section></>}

function Team(){const[items,setItems]=useState<any[]>([]);const[error,setError]=useState("");useEffect(()=>{void listWorkspaceTeam().then(setItems).catch(e=>setError(errorMessage(e,"Unable to load team.")))},[]);return <section className="hlc-launch-grid">{error&&<p role="alert">{error}</p>}{items.map(p=><article key={p.user_id} className="hlc-launch-card"><h2>{p.full_name||"Unnamed member"}</h2><p><strong>Role:</strong> {p.role}</p><p>Onboarding: {p.onboarding_completed?"complete":"in progress"}</p></article>)}{items.length===0&&!error&&<p>No authorized team profiles.</p>}</section>}

function Services(){const[providers,setProviders]=useState<Contractor[]>([]);const[items,setItems]=useState<any[]>([]);const[contractorId,setContractorId]=useState("");const[name,setName]=useState("");const[error,setError]=useState("");async function load(){try{const[p,s]=await Promise.all([listContractors({}),listProviderServices()]);setProviders(p);setItems(s)}catch(e){setError(errorMessage(e,"Unable to load provider services."))}}useEffect(()=>{void load()},[]);async function submit(e:FormEvent){e.preventDefault();try{await createProviderService(Number(contractorId),name);setName("");await load()}catch(r){setError(errorMessage(r,"Unable to save provider service."))}}return <><form onSubmit={submit} className="hlc-launch-card"><h2>Add service capability</h2><label>Provider<select required value={contractorId} onChange={e=>setContractorId(e.target.value)}><option value="">Choose</option>{providers.map(p=><option key={p.id} value={p.id}>{p.company_name||p.contact_name||p.id}</option>)}</select></label><label>Service<input required value={name} onChange={e=>setName(e.target.value)}/></label><button>Save service</button>{error&&<p role="alert">{error}</p>}</form><section className="hlc-launch-card">{items.map(s=><p key={s.id}>{s.service_name} · provider {s.contractor_id}</p>)}{items.length===0&&<p>No service capabilities recorded.</p>}</section></>}

function Analytics(){const[data,setData]=useState<Record<string,number>|null>(null);const[error,setError]=useState("");useEffect(()=>{void getWorkspaceAnalytics().then(setData).catch(e=>setError(errorMessage(e,"Unable to load analytics.")))},[]);if(error)return <p role="alert">{error}</p>;if(!data)return <p role="status">Loading analytics…</p>;return <section className="hlc-launch-grid">{Object.entries(data).map(([key,value])=><article key={key} className="hlc-launch-card"><strong className="hlc-launch-metric-value">{value}</strong><p>{key}</p></article>)}</section>}
function Approvals(){
  ...
  return <div className="hlc-approval-shell">
    {error&&<p role="alert" className="hlc-approval-status is-error">{error}</p>}
    {notice&&<p role="status" className="hlc-approval-status is-success">{notice}</p>}
    {Object.entries(invitationLinks).map(([applicationId,link])=><div key={applicationId} className="hlc-approval-access">
    ...
    <section className="hlc-approval-section" aria-labelledby="professional-review-heading">
      <div className="hlc-approval-heading">
        <div><small className="hlc-approval-eyebrow">PROFESSIONAL NETWORK</small><h2 id="professional-review-heading" className="hlc-approval-title">Applications awaiting your decision</h2></div>
        <strong>{pending.length} pending</strong>
      </div>
      {pending.map(application=><article key={application.id} className="hlc-approval-item">
        <div className="hlc-approval-item-heading">
          <div><small className="hlc-approval-eyebrow">{application.status.replaceAll("_"," ")}</small><h3 className="hlc-approval-title">{application.organization_name}</h3></div>
          <time dateTime={application.created_at}>{new Date(application.created_at).toLocaleString()}</time>
        </div>
        <p className="hlc-approval-contact"><strong>{application.contact_name}</strong> · <a href={`mailto:${application.email}`}>{application.email}</a> · <a href={`tel:${application.phone}`}>{application.phone}</a></p>
        <dl className="hlc-approval-details">
      ...
      {pending.length===0&&!error&&<p className="hlc-approval-empty">No Professional applications are waiting for review.</p>}
    </section>

    <details className="hlc-approval-section">
      <summary className="hlc-approval-summary">Completed Professional decisions ({decided.length})</summary>
      {decided.map(application=><div key={application.id} className="hlc-approval-history">
      ...
      {decided.length===0&&<p className="hlc-approval-empty">No completed Professional decisions.</p>}
    </details>

    <details className="hlc-approval-section">
      <summary className="hlc-approval-summary">Other owner-attention items ({attention.length})</summary>
      {attention.map(item=><div key={item.id} className="hlc-approval-history"><strong>{item.status}</strong><span>{item.reason}</span><small>{item.related_entity_type||"system"} {item.related_entity_id||""}</small></div>)}
      {attention.length===0&&!error&&<p className="hlc-approval-empty">No other owner-attention items.</p>}
    </details>
  </div>
}
function SystemHealth(){const[data,setData]=useState<any>(null);const[error,setError]=useState("");useEffect(()=>{void getSystemHealth().then(setData).catch(e=>setError(errorMessage(e,"Unable to read system health.")))},[]);if(error)return <p role="alert">{error}</p>;if(!data)return <p role="status">Loading health…</p>;return <section className="hlc-launch-grid"><article className="hlc-launch-card"><h2>Communications</h2>{data.providers.map((p:any,i:number)=><p key={i}>{p.provider_name} · {p.channel} · <strong>{p.status}</strong></p>)}{data.providers.length===0&&<p>No provider connections recorded.</p>}</article><article className="hlc-launch-card"><h2>Subscription</h2><p>{data.subscription?.status||"not configured"}</p></article><article className="hlc-launch-card"><h2>Notifications</h2><p>{data.notificationCount} records</p></article><article className="hlc-launch-card"><h2>Recent AI runs</h2><p>{data.recentAgentRuns.length} recorded</p></article></section>}
...
function ProviderSurface({page}:{page:LaunchSurfaceKey}){
  ...
  if(page==="providerDetail") return <>{error&&<p role="alert">{error}</p>}{detail?<article className="hlc-launch-card"><h2>{detail.company_name||detail.contact_name||`Provider ${detail.id}`}</h2><p>{detail.specialty||"Trade not recorded"}</p><p>{[detail.city,detail.state,detail.zip].filter(Boolean).join(", ")||"Location not recorded"}</p><p>Status: {detail.status||"not recorded"}</p>{detail.phone&&<a href={`tel:${detail.phone}`}>Call provider</a>}{detail.email&&<a href={`mailto:${detail.email}`}>Email provider</a>}</article>:!error&&<p>Loading provider…</p>}</>;
  return <>
    <div className="hlc-launch-row"><Link to="/network/service-areas">Service areas</Link><Link to="/network/availability">Availability</Link><Link to="/network/saved">Saved</Link><Link to="/contractor-portal/services">Services</Link></div>
    {error&&<p role="alert">{error}</p>}
    <section className="hlc-launch-grid">
      {visible.map(p=>{
