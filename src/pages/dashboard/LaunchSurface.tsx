...
function ProviderSurface({page}:{page:LaunchSurfaceKey}){
  const {providerId}=useParams(); const[providers,setProviders]=useState<Contractor[]>([]); const[areas,setAreas]=useState<any[]>([]); const[availability,setAvailability]=useState<any[]>([]); const[saved,setSaved]=useState<Set<number>>(new Set()); const[detail,setDetail]=useState<Contractor|null>(null); const[error,setError]=useState("");
  ...
  async function addArea(id:number){try{await saveServiceArea({contractorId:id,state:"PA"});await load()}catch(r){setError(errorMessage(r,"Unable to save service area."))}}
  async function setAvail(id:number){try{await saveProviderAvailability({contractorId:id,available:true,note:"Available"});await load()}catch(r){setError(errorMessage(r,"Unable to update availability."))}}
