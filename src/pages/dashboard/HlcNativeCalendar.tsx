...
const appointmentStatusLabels = { scheduled: "Scheduled", completed: "Completed", cancelled: "Cancelled", no_show: "No-show" } as const;
function startOfDay(date: Date) { const next = new Date(date); next.setHours(0, 0, 0, 0); return next; }
function startOfWeek(date: Date) { const next = startOfDay(date); const day = next.getDay(); next.setDate(next.getDate() + (day === 0 ? -6 : 1 - day)); return next; }
function sameDay(a: Date, b: Date) { return a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth() && a.getDate() === b.getDate(); }
function sameMonth(a: Date, b: Date) { return a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth(); }
function inSelectedPeriod(value: string, selectedDate: Date, view: CalendarView) { const date = new Date(value); if (view === "day") return sameDay(date, selectedDate); if (view === "month") return sameMonth(date, selectedDate); const start = startOfWeek(selectedDate); const end = new Date(start); end.setDate(end.getDate() + 7); return date >= start && date < end; }
...
export default function HlcNativeCalendar() {
  ...
  const allItems = useMemo<ScheduleItem[]>(() => [
    ...appointments.map((appointment) => ({ kind: "appointment" as const, id: appointment.id, start: appointment.appointment_date, end: appointment.appointment_end_at, title: appointment.job?.name || `Job ${appointment.job_id}`, subtitle: contractorName(appointment), status: appointment.status, notes: appointment.notes })),
    ...nativeEvents.map((event) => ({ kind: "event" as const, id: event.id, start: event.start_at, end: event.end_at, title: event.title, subtitle: eventTypeLabel(event.event_type), status: event.status, notes: event.description })),
  ...
  async function reschedule(start: string, end: string) { if (!rescheduling) return; const ok = await run(() => rescheduleAppointment(rescheduling.id, start, end), "Appointment rescheduled."); if (ok) setRescheduling(null); }
  ...
  async function createEvent(event: FormEvent<HTMLFormElement>) {
    ...
    const ok = await run(() => createHlcCalendarEvent({ title: String(form.get("title")), description: String(form.get("description") || ""), startAt: new Date(start).toISOString(), endAt: new Date(end).toISOString(), eventType: String(form.get("eventType")) as HlcCalendarEventType }), "HLC calendar event created.");
    if (ok) { setNewEventOpen(false); setSelectedDate(startOfDay(new Date(start))); setView("day"); }
  }

  function moveDate(direction: -1 | 1) { const next = new Date(selectedDate); if (view === "day") next.setDate(next.getDate() + direction); if (view === "week") next.setDate(next.getDate() + direction * 7); if (view === "month") next.setMonth(next.getMonth() + direction); setSelectedDate(startOfDay(next)); setSelected(null); }

  const weekStart = startOfWeek(today); const weekEnd = new Date(weekStart); weekEnd.setDate(weekEnd.getDate() + 7);
  ...
  const monthGrid = useMemo(() => { const first = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), 1); const last = new Date(selectedDate.getFullYear(), selectedDate.getMonth() + 1, 0); const mondayIndex = first.getDay() === 0 ? 6 : first.getDay() - 1; const days: Array<Date | null> = Array.from({ length: mondayIndex }, () => null); for (let day = 1; day <= last.getDate(); day += 1) days.push(new Date(selectedDate.getFullYear(), selectedDate.getMonth(), day)); while (days.length % 7 !== 0) days.push(null); return days; }, [selectedDate]);
  const defaultStart = useMemo(() => { const value = new Date(selectedDate); value.setHours(9, 0, 0, 0); return localInputValue(value); }, [selectedDate]);
  const defaultEnd = useMemo(() => { const value = new Date(selectedDate); value.setHours(10, 0, 0, 0); return localInputValue(value); }, [selectedDate]);
  ...
  return <main className="hlc-calendar-page hlc-native-calendar">
    ...
    {loading && <div className="hlc-calendar-banner">Loading HLC Calendar…</div>}{error && <div className="hlc-calendar-banner error" role="alert">{error}</div>}{message && <div className="hlc-calendar-banner success" role="status">{message}</div>}
    ...
    <section className="hlc-calendar-workspace"><div className="hlc-calendar-board"><div className="hlc-calendar-toolbar"><div className="hlc-calendar-nav-controls"><button type="button" onClick={() => { setSelectedDate(today); setSelected(null); }}>Today</button><button type="button" onClick={() => moveDate(-1)}>‹</button><button type="button" onClick={() => moveDate(1)}>›</button><div className="hlc-calendar-date-label"><span>◫</span>{formatDate(selectedDate)}</div></div><div className="hlc-calendar-view-switch">{(["day", "week", "month"] as CalendarView[]).map((option) => <button key={option} type="button" className={view === option ? "active" : ""} onClick={() => { setView(option); setSelected(null); }}>{option[0].toUpperCase() + option.slice(1)}</button>)}</div></div><div className="hlc-calendar-period-label"><span>{view} schedule</span><strong>{visibleItems.length} {visibleItems.length === 1 ? "item" : "items"}</strong></div><div className="hlc-calendar-timeline">{!loading && visibleItems.length === 0 && <div className="hlc-calendar-empty"><span>◫</span><h2>No calendar items here</h2><p>Create an HLC event or schedule a job appointment.</p><button type="button" onClick={() => setNewEventOpen(true)}>Create event</button></div>}{visibleItems.map((item) => { const active = selected?.kind === item.kind && selected.id === item.id; return <button type="button" key={`${item.kind}-${item.id}`} className={`hlc-calendar-event status-${item.status}${item.kind === "event" ? " native" : ""}${active ? " selected" : ""}`} onClick={() => setSelected({ kind: item.kind, id: item.id } as SelectedItem)}><div className="hlc-calendar-event-time"><strong>{formatTime(item.start)}</strong><span>{item.end ? formatTime(item.end) : "Open end"}</span></div><div className="hlc-calendar-event-copy"><div className="hlc-calendar-event-heading"><strong>{item.title}</strong><span className={`hlc-calendar-status status-${item.status}`}>{item.kind === "event" ? "HLC event" : item.status}</span></div><span>{item.subtitle}</span>{item.notes && <small>{item.notes}</small>}</div><span className="hlc-calendar-event-arrow">→</span></button>; })}</div></div>
      <aside className="hlc-calendar-detail-panel">{selectedAppointment ? <><div className="hlc-calendar-detail-heading"><span className={`hlc-calendar-status status-${selectedAppointment.status}`}>{appointmentStatusLabels[selectedAppointment.status]}</span><h2>{selectedAppointment.job?.name || `Job ${selectedAppointment.job_id}`}</h2><p>{contractorName(selectedAppointment)}</p></div><div className="hlc-calendar-detail-actions"><Link className="primary" to={`/jobs/${selectedAppointment.job_id}`}>View job details <span>→</span></Link>{selectedAppointment.status === "scheduled" && <><button disabled={busy || !selectedAppointment.appointment_end_at} onClick={() => setRescheduling(selectedAppointment)}>Reschedule</button><button disabled={busy} onClick={() => void run(() => completeAppointment(selectedAppointment.id), "Appointment completed.")}>Complete appointment</button><button className="danger" disabled={busy} onClick={() => void run(() => cancelAppointment(selectedAppointment.id), "Appointment cancelled.")}>Cancel appointment</button><button className="danger muted" disabled={busy} onClick={() => void run(() => markNoShow(selectedAppointment.id), "Appointment marked no-show.")}>Mark no-show</button></>}</div><dl className="hlc-calendar-detail-list"><div><dt>Date</dt><dd>{formatDate(new Date(selectedAppointment.appointment_date))}</dd></div><div><dt>Time</dt><dd>{formatTime(selectedAppointment.appointment_date)} – {selectedAppointment.appointment_end_at ? formatTime(selectedAppointment.appointment_end_at) : "Open end"}</dd></div><div><dt>Contractor</dt><dd>{contractorName(selectedAppointment)}</dd></div><div><dt>Calendar</dt><dd>HLC native schedule</dd></div>{selectedAppointment.notes && <div><dt>Notes</dt><dd>{selectedAppointment.notes}</dd></div>}</dl></> : selectedEvent ? <><div className="hlc-calendar-detail-heading"><span className={`hlc-calendar-status status-${selectedEvent.status}`}>{eventTypeLabel(selectedEvent.event_type)}</span><h2>{selectedEvent.title}</h2><p>HLC workspace event</p></div><div className="hlc-calendar-detail-actions">{selectedEvent.status === "scheduled" && <button disabled={busy} onClick={() => void run(() => setHlcCalendarEventStatus(selectedEvent.id, "completed"), "HLC event completed.")}>Mark complete</button>}{selectedEvent.status === "scheduled" && <button className="danger" disabled={busy} onClick={() => void run(() => setHlcCalendarEventStatus(selectedEvent.id, "cancelled"), "HLC event cancelled.")}>Cancel event</button>}<button className="danger muted" disabled={busy} onClick={() => void run(async () => { await deleteHlcCalendarEvent(selectedEvent.id); setSelected(null); }, "HLC event deleted.")}>Delete event</button></div><dl className="hlc-calendar-detail-list"><div><dt>Date</dt><dd>{formatDate(new Date(selectedEvent.start_at))}</dd></div><div><dt>Time</dt><dd>{formatTime(selectedEvent.start_at)} – {formatTime(selectedEvent.end_at)}</dd></div><div><dt>Type</dt><dd>{eventTypeLabel(selectedEvent.event_type)}</dd></div><div><dt>Status</dt><dd>{selectedEvent.status}</dd></div>{selectedEvent.description && <div><dt>Notes</dt><dd>{selectedEvent.description}</dd></div>}</dl></> : <div className="hlc-calendar-detail-empty"><span>◫</span><h2>HLC Calendar</h2><p>Select an appointment or native event to manage it.</p><button type="button" onClick={() => setNewEventOpen(true)}>+ New event</button></div>}
        <div className="hlc-calendar-mini-month"><div className="hlc-calendar-mini-heading"><button type="button" onClick={() => moveDate(-1)}>‹</button><strong>{new Intl.DateTimeFormat(undefined, { month: "long", year: "numeric" }).format(selectedDate)}</strong><button type="button" onClick={() => moveDate(1)}>›</button></div><div className="hlc-calendar-mini-weekdays">{["M","T","W","T","F","S","S"].map((day, index) => <span key={`${day}-${index}`}>{day}</span>)}</div><div className="hlc-calendar-mini-grid">{monthGrid.map((date, index) => date ? <button type="button" key={date.toISOString()} className={`${sameDay(date, selectedDate) ? "selected" : ""}${sameDay(date, today) ? " today" : ""}`} onClick={() => { setSelectedDate(startOfDay(date)); setView("day"); setSelected(null); }}>{date.getDate()}</button> : <span key={`empty-${index}`} />)}</div></div>
      </aside></section>
  </main>;
}
