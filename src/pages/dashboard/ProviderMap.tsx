...
export default function ProviderMap() {
  ...
  return <main style={pageStyle}>
    <header style={heroStyle}>
      <p style={eyebrowStyle}>HLC Network · provider location records</p>
      <h1 style={{ margin: 0 }}>Provider map</h1>
      <p style={{ marginBottom: 0 }}>Pins use only coordinates stored in HLC. Approximate city or ZIP-area points are labeled separately from verified exact locations. This map does not calculate nearest providers, ETA, routing, dispatch, or live location.</p>
    </header>
      ...
      <section style={listPanelStyle} aria-label="Provider map list">
        ...
        <div style={providerListStyle}>
          ...
          >
            ...
            <small>{hasCoordinates(provider) ? coordinateLabel(provider) : "Location not mapped yet"}</small>
          </button>)}
    ...
    {selected && <section style={detailStyle} aria-label="Selected provider map details">
      <div>
        ...
        {hasCoordinates(selected) && <p style={accuracyNoteStyle}><strong>{coordinateLabel(selected)}.</strong> {coordinateDescription(selected)}</p>}
        <div style={navStyle}>
      ...
      {canManageCoordinates && <form onSubmit={saveCoordinates} style={coordinateFormStyle}>
        <div><strong>Management map coordinates</strong><p style={{ margin: "4px 0 0", color: "#64748b" }}>Enter coordinates only when the source confirms the provider's exact business/location point. Saving through this control marks the coordinates as verified; do not replace an approximate city/ZIP point with another inferred point.</p></div>
        ...
        <button type="submit" disabled={busy || !latitude || !longitude} style={primaryButtonStyle}>{busy ? "Saving location…" : "Save verified map location"}</button>
      </form>}
    </section>}
  </main>;
}
...
const navStyle = { display: "flex", flexWrap: "wrap" as const, gap: 14, alignItems: "center", justifyContent: "center" };
const filterStyle = { display: "flex", gap: 16, flexWrap: "wrap" as const, alignItems: "end", justifyContent: "center", padding: 16, border: "1px solid #cbd5e1", borderRadius: 14, background: "#fff" };
const fieldStyle = { display: "grid", gap: 6, minWidth: 220, fontWeight: 700 };
const checkboxStyle = { minHeight: 44, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontWeight: 700 };
...
const mapHeaderStyle = { display: "flex", justifyContent: "center", gap: 16, flexWrap: "wrap" as const, padding: "12px 16px", borderBottom: "1px solid #e2e8f0", color: "#334155" };
...
const accuracyNoteStyle = { maxWidth: 760, margin: "10px auto", padding: 12, border: "1px solid #cbd5e1", borderRadius: 12, background: "#f8fafc", color: "#475569" };
...
const coordinateFormStyle = { display: "flex", gap: 14, flexWrap: "wrap" as const, alignItems: "end", justifyContent: "center", paddingTop: 16, borderTop: "1px solid #e2e8f0" };
